"""Fetch Danish vernacular names for every Chiroptera species from GBIF.

GBIF aggregates vernacular names from many taxonomic checklists/sources,
including (for Danish bats specifically) the "National Checklist of all
species occurring in Denmark". We query by scientific name, then pull any
Danish-language ("dan") vernacular name attached to the matched taxon.
Species with no Danish name recorded anywhere in GBIF's sources are simply
omitted from the output -- we do not invent names.
"""
import json
import time
import urllib.parse
...
OUT = Path(__file__).parent / "danish_names.json"
...
def danish_name_for(sci_name_space, fallback_name_space=None):
    ...
    if (not match or "usageKey" not in match) and fallback_name_space:
        ...
        # (e.g. recent splits like Eptesicus -> Cnephaeus); retry with the
        # MSW3-era name, which GBIF's taxonomy generally does recognise.
